...
如果你的FFmpeg版本**较新**，还可以使用Vulkan API进行硬件解码：

```bash
ffmpeg -hwaccel vulkan -hwaccel_output_format vulkan [...]
```

在Vulkan与VAAPI同时可用的时候，一般**Vulkan API的效率明显更高**；但目前Vulkan API的支持覆盖度相比VAAPI要低。截至2025.07.27，在Vulkan `1.4.321.0`中，已支持HEVC、H264、AV1、VP9的硬件解码，但VP9目前仅支持AMD RDNA 1及以上显卡，较旧的硬件即使支持VP9解码也无法在Vulkan API中使用。
