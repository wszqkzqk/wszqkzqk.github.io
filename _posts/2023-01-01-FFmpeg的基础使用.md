...
```bash
ffmpeg -init_hw_device vulkan=vkdev:0 -filter_hw_device vkdev -hwaccel vulkan [...]
```
...
需要注意的是，对于VAAPI或者D3D11VA这样的硬件加速API，如果遇到驱动支持但是硬件不支持的情况，FFmpeg会自动回退到软件解码。然而，截至2025.08.06，Vulkan在这种情况下不会自动回退到软件解码，而是会得到大量错误输出（但并不会报错退出，会输出不可解码的文件），因此需要在使用Vulkan解码时确保硬件支持所需的解码格式。

`-hwaccel auto`目前不会自动选择使用Vulkan API，也不会使用Android的MediaCodec API进行硬件加速解码。
...
其中`-hwaccel_output_format vulkan`用于指定Vulkan的输出颜色空间。Vulkan提供的硬件编码支持一般**效率更高**。截至2025.07.27，Vulkan API的硬件编码支持格式仍然局限于**HEVC、H264、AV1**，其他格式仍然不支持。
...
### Android平台：在Termux中使用MediaCodec硬件加速

详见笔者的另一篇博客：[利用Android手机硬件进行高效灵活的编解码](https://wszqkzqk.github.io/2025/10/26/Termux-Hardware-Acceleration/)。基础命令与上文类似：、
