...
`-hwaccel_output_format`和`-vf 'hwupload,scale_vaapi=format=nv12'`在这里的作用是相同的，但后者更加独立灵活，可以适用于更多场合，在下文中有仅能够使用`-vf 'hwupload,scale_vaapi=format=nv12'`而不能使用`-hwaccel_output_format`的示例。 
