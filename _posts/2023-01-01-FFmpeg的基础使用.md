...
如果你的FFmpeg版本**较新**，还可以使用Vulkan API进行硬件解码。Vulkan是**跨平台**的API，支持多种操作系统和硬件平台：

```bash
ffmpeg -hwaccel vulkan -hwaccel_output_format vulkan [...]
```

在Vulkan与VAAPI或者D3D11VA这样的较老的硬件加速API同时可用的时候，一般**Vulkan API的效率明显更高**，而且Vulkan也是目前几乎唯一**在通过Wine运行的FFmpeg中直接可用**的硬件加速方法（仅硬件解码，硬件编码仍然不可用）。但目前Vulkan API的支持覆盖度相比VAAPI要低：截至2025.07.27，在Vulkan `1.4.321.0`中，已支持HEVC、H264、AV1、VP9的硬件解码，但VP9目前仅支持AMD RDNA 1及以上显卡，较旧的硬件即使支持VP9解码也无法在Vulkan API中使用。
...
这样，FFmpeg会根据平台和硬件配置自行选择可用的硬件加速API。（但是目前Vulkan API仍然需要手动指定）
...
同样，对于**较新**的FFmpeg版本，也可以使用Vulkan API进行硬件编码：
