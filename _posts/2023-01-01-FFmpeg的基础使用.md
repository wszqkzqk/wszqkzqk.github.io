...
如果不加入`-hwaccel_output_format`参数，则需要手动传递`-vf`参数指定颜色空间：

```bash
ffmpeg -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 [...] -vf 'hwupload,scale_vaapi=format=nv12' -c:v hevc_vaapi [...]
```

除了`nv12`外，也可以使用更多硬件支持的颜色空间。与`-hwaccel_output_format`相比，`-vf 'hwupload,scale_vaapi=format=nv12'`后者更加独立灵活，可以适用于更多场合，在下文中有仅能够使用`-vf 'hwupload,scale_vaapi=format=nv12'`而不能使用`-hwaccel_output_format`的示例。实际上，`hwupload`表示加载到硬件加速设备上，`hwdownload`则表示从硬件加速设备上下载，这一点在后面有关滤镜的操作上也很重要，凡是使用硬件加速设备的滤镜之前，都需要添加`hwupload`，而从硬件设备切换到CPU时，则需要添加`hwdownlaod`。
...
```bash
ffmpeg -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 [...] -c:v hevc_vaapi -vf 'subtitles=example.srt,hwupload,scale_vaapi=format=nv12' [...]
```

其中，`hwupload,scale_vaapi=format=nv12`必须要跟在最后。
...
```bash
ffmpeg [...] -vf eq=saturation=1.7:brightness=1.1:contrast=1.1 [...]
```

这个例子的含义是，将视频的饱和度提高到170%，亮度提高到110%，对比度提高到110%。
...
f
```bash
ffmpeg -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 [...] -vf 'hwupload,scale_vaapi=format=nv12,scale_vaapi=w=[输出宽度]:h=[输出高度]' [...] -c:v hevc_vaapi [...]
```

`scale_vaapi`可以在硬件加速的情况下对视频或图片进行缩放。

* 需要使用`-hwaccel vaapi`、`-hwaccel_output_format`（或者`-vf hwupload,scale_vaapi=format=nv12`）、`vaapi -vaapi_device /dev/dri/renderD128`等选项指定硬件加速的设备和格式
