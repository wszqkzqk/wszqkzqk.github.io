...
在Vulkan与VAAPI或者D3D11VA这样的较老的硬件加速API同时可用的时候，一般**Vulkan API的效率明显更高**，而且Vulkan也是目前几乎唯一**在通过Wine运行的FFmpeg中直接可用**的硬件加速方法。但目前Vulkan API的支持覆盖度相比VAAPI要低：截至2025.07.27，在Vulkan `1.4.321.0`中，已支持HEVC、H264、AV1、VP9的硬件解码，但VP9目前仅支持AMD RDNA 1及以上显卡，较旧的硬件即使支持VP9解码也无法在Vulkan API中使用。
