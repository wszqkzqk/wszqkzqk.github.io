...
`-hwaccel_output_format`和`-vf 'hwupload,scale_vaapi=format=nv12'`在这里的作用是相同的，但后者更加独立灵活，可以适用于更多场合，在下文中有仅能够使用`-vf 'hwupload,scale_vaapi=format=nv12'`而不能使用`-hwaccel_output_format`的示例。实际上，`hwupload`表示加载到硬件加速设备上，`hwdownload`则表示从硬件加速设备上下载，这一点在后面有关滤镜的操作上也很重要，凡是使用硬件加速设备的滤镜之前，都需要添加`hwupload`，而从硬件设备切换到CPU时，则需要添加`hwdownlaod`。
