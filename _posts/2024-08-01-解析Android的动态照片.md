...
目前笔者已经将`MotionPhoto`类的实现改进。`MotionPhoto`类本身是一个抽象类，其实现类包括`MotionPhotoFFmpeg`和`MotionPhotoGst`，分别使用`FFmpeg`和`GStreamer`实现了逐帧提取功能。这样可以根据需要选择不同的实现。
