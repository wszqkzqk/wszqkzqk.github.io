...
使用简单的GNU工具很难实现全面的处理功能，也难以用于动态照片的合成。因此，笔者使用Vala语言开发了一个强大的动态照片处理工具——[Live Photo Converter](https://github.com/wszqkzqk/live-photo-conv)，借助**GExiv2的元数据解析**功能，这一工具全面集成了动态照片的**提取、编辑、合成**等功能。此外笔者还实现了**可供选择的GStreamer/FFmpeg双后端**的支持，可将动态照片中的嵌入视频的内容**逐帧导出**为图片。笔者还为项目设计了易用的CLI。该工具支持Linux与Windows平台，功能更为强大、方便。
