...
Android的动态照片是一种越来越普及的媒体文件格式，它可以将包含音频的视频与静态图片结合在一起，形成一个包含拍摄时动态视听信息的照片。相比于静态照片，动态照片可以利用**嵌入视频**带来更**生动的感官体验**；而相比于普通的视频，动态照片包含了一个使用**更好的算法生成的、像素更高、质量更好**的**静态图片**，更适合当作图片来保存和分享。因此，在假日出行、亲友聚会与自然风光等场景下，动态照片可以更好地记录和传达当时的氛围与情感。
...
笔者使用Vala语言开发了一个强大的动态照片处理工具——[Live Photo Converter](https://github.com/wszqkzqk/live-photo-conv)，借助**GExiv2的元数据解析**功能，全面集成了动态照片的**提取、编辑、合成**等功能。此外笔者还实现了**可供选择的GStreamer/FFmpeg双后端**的支持，可将动态照片中的嵌入视频的内容**逐帧导出**为图片。笔者还为项目设计了多个方便好用的CLI。该工具支持Linux与Windows平台，功能更为强大、方便。
