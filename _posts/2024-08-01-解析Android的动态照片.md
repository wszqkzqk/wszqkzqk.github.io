...
### 架构：用于提取、解包的[`MotionPhoto`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphoto.vala)

* 内容[过期](#架构改进motionphoto类实现)

目前的工具使用`MotionPhoto`类来表示动态照片，该类没有公开字段，仅包含以下私有字段：
...
在创建`MotionPhoto`类的实例时，程序会调用`get_video_offset`方法，这一方法的实现如下：
...
在创建`MotionPhoto`类的实例后，可以调用`extract_main_image`方法提取静态图片，该方法接受一个可选的目标路径参数：
...
### 架构：用于合成的[`MotionMaker`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionmaker.vala)

`MotionMaker`类用于合成动态照片，该类包含以下私有字段：
...
在创建`MotionMaker`类的实例时，程序会根据传入的参数生成输出路径，如果需要导出原始`XMP`元数据，则会将主图片的元数据复制到输出文件中。
...
* 如果需要导出原始`XMP`元数据，则将元数据保存到输出文件中
  * 设置`XMP`元数据中的`MotionPhoto`相关标签

### 架构：用于处理的[`Utils`命名空间](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/utils.vala)
...
### 架构改进：[`MotionPhoto`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphoto.vala)实现

目前笔者已经将`MotionPhoto`类的实现改进。`MotionPhoto`类本身是一个抽象类，其实现类包括[`MotionPhotoFFmpeg`](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphotoffmpeg.vala)和[`MotionPhotoGst`](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphotogst.vala)，分别使用`FFmpeg`和`GStreamer`实现了逐帧提取功能。这样可以根据需要选择不同的实现。

`MotionPhoto`类的其他实现不变，但`splites_images_from_video`变为了抽象方法，需要在实现类中实现：

```vala
public abstract void splites_images_from_video (string? output_format = null, string? dest_dir = null) throws Error;
```

[`MotionPhotoFFmpeg`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphotoffmpeg.vala)的实现如下：
...
这与原来的实现基本相同，只是将`splites_images_from_video_ffmpeg`方法的实现移到了`MotionPhotoFFmpeg`类中。

[`MotionPhotoGst`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphotogst.vala)的实现如下：
...
### 架构改进（2024.08.26）：[`MotionPhotoFFmpeg`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphotoffmpeg.vala)

笔者对`MotionPhotoFFmpeg`类的实现进行了改进，主要是将`splites_images_from_video`方法中的代码进行了重构，将文件写入的部分与元数据保存的部分分开，这样可以更好地处理异常情况。
...
### 架构改进（2024.08.26）：[`MotionPhotoGst`类](https://github.com/wszqkzqk/motion-photo-conv/blob/main/src/motionphotogst.vala)

笔者对`MotionPhotoGst`类的实现进行了改进，更改了`pipeline`的内容，实现了根据元数据自动旋转视频的功能，这样可以避免在导出图片时出现方向不正确的问题。
