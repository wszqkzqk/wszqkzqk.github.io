...
文件从开始到`$FILE_SIZE - 视频大小`的部分为完整静态图片，从`$FILE_SIZE - 视频大小`到文件末尾的部分为完整视频文件（需删除原图中的XMP元数据）。
...
目前笔者已经使用GStreamer实现了逐帧提取功能，也可以选择使用FFmpeg。其中GStreamer在构建时为可选功能，在程序中通过动态链接使用；FFmpeg在程序内部则通过单独的进程调用CLI来使用。

有关这一程序的设计，可以在AI生成的DeepWiki上查看，也可以向AI助手询问：[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/wszqkzqk/live-photo-conv)
