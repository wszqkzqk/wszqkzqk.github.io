...
# 增强：自动获取位置和时区（基于网络IP地理定位）

在前述 Libadwaita 版本的基础上，我们可以进一步提升用户体验：**自动获取本机的地理位置和时区**，让用户无需手动输入即可一键查看本地太阳高度角变化曲线。在这个例子中，我们将进一步介绍 **JSON 数据解析**、**异步网络请求**以及 GTK4/Libadwaita 的更多高级 UI 组件使用，以及使用对话框向用户展示错误信息。
...
| [![#~/img/GTK-examples/pku-light-solar-angle-250807.webp](/img/GTK-examples/pku-light-solar-angle-250807.webp)](/img/GTK-examples/pku-light-solar-angle-250807.webp) | [![#~/img/GTK-examples/pku-dark-solar-angle-250807.webp](/img/GTK-examples/pku-dark-solar-angle-250807.webp)](/img/GTK-examples/pku-dark-solar-angle-250807.webp)
 |:--: | :--: |
|自动获取位置与时区（浅色模式）|自动获取位置与时区（深色模式）|

## 增强点与实现思路

这个增强版本在用户体验和功能上迈出了重要一步，主要体现在以下几个方面：

* 自动获取地理位置：
    * 不依赖 GeoClue 等可能需要特定权限或系统服务的技术（这在某些特定系统环境或沙箱应用中可能存在权限问题）。
    * 通过调用**免费的在线 IP 地理定位服务**（本例中使用 `ipapi.co`）来实现。应用将发起异步网络请求，解析返回的 JSON 数据以获取经纬度。这种方式具有较好的跨平台兼容性，其精度依赖于 IP 数据库，可能不如 GPS 或其他高精度定位服务准确，但一般电脑可能没有 GPS 硬件，而且计算太阳高度角时，IP 定位的精度通常足够满足需求。
* 自动获取本地时区：
    * 直接利用 `GLib.TimeZone` 提供的 API 获取系统当前的本地时区偏移量（与 UTC 的差值）。这是一种稳定且精确的方式，适用于各种 Linux 桌面环境，并且能正确处理夏令时。
* 界面增强与用户反馈：
    * 在“位置与时间设置”分组顶部增加了一个“自动检测位置”按钮。
    * 点击按钮后，按钮本身会切换到加载状态（显示 `Gtk.Spinner`），并使用 `Gtk.Stack` 容器进行平滑的过渡动画，避免界面元素在加载时跳动，从而提供即时且不突兀的用户反馈。
      * 如果没有使用 `Gtk.Stack`，由于 `Gtk.Spinner` 的**大小与原来的按钮不同**，会导致界面在加载时出现**抖动或重排**现象，影响用户体验。
      * `Gtk.Stack` 是解决界面大小变化问题的理想选择。
    * 定位成功后，经纬度和时区输入框会自动填充新值，并立即刷新图表。
    * 如果定位失败（例如网络问题或服务返回错误），程序会弹出一个 `Adw.AlertDialog` 提示用户具体的错误信息，提升了应用的健壮性和用户体验。
    * 无论成功与否，按钮都会在尝试结束后恢复到初始状态，允许用户再次尝试。

## 关键技术点

为了实现上述增强，这个示例引入了以下关键技术：

* 异步网络请求与 `json-glib` 解析：
    * Vala 的 `async` 异步编程模型是实现不阻塞 UI 的网络请求的关键。通过 `Gio.File.new_for_uri()` 创建一个指向 `ipapi.co` API 的 `File` 对象，然后利用 `yield file.read_async()` 进行**异步数据读取**，这会在后台执行网络操作，直到数据可用。
    * 接收到原始文本数据后，通过 `json-glib` 库中的 `Json.Parser` 和 `Json.Object` 来解析 JSON 字符串，安全地提取 `latitude`、`longitude` 以及可能的错误信息。对返回结果进行详细的错误检查，例如判断是否包含 `"error": true` 字段，以确保数据的有效性。
* UI 更新与主线程调度：
    * 网络请求和 JSON 解析完成后，获取到的数据需要用来更新用户界面（例如，设置 `Adw.SpinRow` 的 `value`）。
    * 为了确保这些 UI 操作在**主线程**上执行（GTK/Libadwaita 控件的操作必须在主线程进行），我们使用 `GLib.Idle.add()` 将 UI 更新的逻辑调度回主循环。这种模式确保了 UI 的响应性，避免了在等待网络请求时界面冻结。
* 错误处理与用户反馈：
    * `try-catch` 结构广泛应用于网络请求和 JSON 解析过程中，捕获可能发生的 `IOError`。
    * 捕获到的错误信息会传递给 `show_location_error` 方法，该方法负责创建一个 `Adw.AlertDialog` 并向用户显示友好的错误消息，提升了应用的健壮性。`Adw.AlertDialog` 是 Libadwaita 推荐的模态对话框，其设计风格与 GNOME 桌面环境高度统一。
* 平滑过渡效果：
    * `Gtk.Stack` 控件用于管理“自动检测”按钮和加载指示器 (`Gtk.Spinner`)。通过设置 `transition_type = Gtk.StackTransitionType.CROSSFADE`，可以在它们之间实现平滑的淡入淡出效果，极大地提升了用户感知的流畅性。
    * `hhomogeneous` 和 `vhomogeneous` 属性有助于确保 `Gtk.Stack` 在不同子控件切换时保持相同的尺寸，**避免界面的抖动和重排**。

## 实现代码

增强后的 `solarangle.vala` 完整代码如下，它兼容了之前所有功能并新增了自动定位能力：
...
编译和运行此增强版应用程序需要确保系统已安装 Vala、GTK4、LibAdwaita，以及用于 JSON 解析的 `json-glib` 库，以及 C 编译工具（如 `gcc`）。
...
* 安装依赖（Windows MSYS2）：
  ...
  ```
* 命令：
  在文件最上方的 shebang (`#!/usr/bin/env -S vala ...`) 已经包含了所需的编译选项。如果您想手动编译（假设文件名为 `solarangle.vala`），可以使用以下命令：
  ```bash
  valac --pkg=gtk4 --pkg=libadwaita-1 --pkg=json-glib-1.0 -X -lm -X -pipe -X -O2 solarangle.vala
  ```
  编译成功后，直接运行生成的可执行文件即可。如果在 Linux 下并赋予了执行权限 (`chmod +x solarangle.vala`)，可以直接运行脚本：
  ```bash
  ./solarangle.vala
  ```
