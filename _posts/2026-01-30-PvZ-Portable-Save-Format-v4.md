...
subtitle:     一个跨架构、跨平台且完美保存关卡内状态的游戏存档系统
date:         2026-01-30
author:       wszqkzqk
header-img:   img/bg-pvz.webp
catalog:      true
tags:         C++ 游戏移植 序列化 游戏存档 开源软件
---
...
实现“跨平台”不仅仅意味着让代码在 Linux、Windows、macOS 上编译通过，更意味着**用户体验的一致性**。而这其中最关键的一环，就是**存档的互通性**。
...
《植物大战僵尸》原版的存档机制设计非常简单粗暴。当你点击“保存并退出”时，游戏基本上是直接把内存中的对象 `memcpy` 到了硬盘上。
...
这种“内存转储（Memory Dump）”的方式对于单一平台的闭源商业游戏来说，开发效率极高，运行速度也快（无需序列化开销）。但对于一个跨平台移植项目来说，这是灾难性的：
...
为了应对未来游戏更新可能增减字段的情况，不仅仅是将属性按顺序写入（这样如果中间加一个字段旧存档就废了），而是给每个关键属性分配一个 **Tag ID**。
...
2.  **YAML 转 v4 (Import)**：
    ...
    *   脚本会自动处理 CRC32 校验和与文件头，生成的存档可以直接在游戏中加载。
...
通过这个工具，即使不了解二进制格式细节，玩家也可以轻松实现“存档修改器”的功能，或者用于制作特殊的测试关卡。
...
本节给出 v4 存档的**可实现级别**结构文档，便于第三方实现读写器。
...
*   常用字段以 `FieldID` 固定编号写入；复杂对象的“尾部字段”使用 `PORTABLE_FIELD_TAIL` 记录。
