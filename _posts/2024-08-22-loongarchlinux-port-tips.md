...
这种问题可以优先**向Arch Linux上游反馈**，在确认x86_64环境当前同样存在该问题后，提交issue，告知该软件包在新版本的依赖下无法构建，让Arch Linux上游进行修复。（此时一般不需要提及移植架构）如果上游没有及时修复且暂缓升级不影响功能，也可以暂缓构建。如果必须要构建，可以尝试在`local-repo`环境中集成该依赖升级前的环境，保证各个向关联的包版本匹配，然后再进行构建。
