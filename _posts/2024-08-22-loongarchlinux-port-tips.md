...
* `-bios ./QEMU_EFI.fd`：指定QEMU使用的EFI固件
  ...
  * 如果不想在镜像站中手动寻找或者手动拉取，可以通过这条命令**一键下载**`edk2-loongarch64`软件包：
    ```bash
...
如果在启动早期（例如加载了initramfs后）即发生内核panic，日志难以查看，可以在内核参数中加入`console=ttyS0`来将日志输出到串口，随后可以在启动QEMU的终端中查看日志。例如，如果使用的是GRUB引导，可以在GRUB的启动项中按`e`键编辑内核参数，找到`linux`开头这行，在末尾添加`console=ttyS0`，然后按`Ctrl+X`启动即可。
...
有时候我们会遇到一些看起来架构无关的软件包在构建时出现依赖错误，而上游并没有这一问题。这种情况一般是我们的实际构建顺序与上游不同导致的：例如上游可能使用了更低版本的依赖来构建，随后将软件包一直放在testing仓库中，等软件包进入core/extra仓库后，依赖版本已经升级了，导致我们在构建时遇到了版本不兼容的问题。
...
这种问题可以优先**向Arch Linux上游反馈**，在**确认x86_64环境当前同样存在该问题**后，提交issue，告知该软件包在新版本的依赖下无法构建，让Arch Linux上游进行修复。（此时一般不需要提及移植架构）如果上游没有及时修复且暂缓升级不影响功能，也可以暂缓构建。如果必须要构建，可以尝试在[`local-repo`环境](https://wszqkzqk.github.io/2024/09/19/build-order-local-repo/)中集成该依赖升级前的环境，保证各个向关联的包版本匹配，然后再进行构建。
