...
- 可以在仓库的[`Release`（点击跳转）](https://github.com/wszqkzqk/pypvz/releases)页面中找到对应版本下载
  - 使用GCC编译
  - 程序包含名称、版本等信息
  - 得到的验证最多
  - `0.7.25.0`之后的部分版本被标注成了`pre-release`，实际上仍然为普通版本，按需下载即可
  - 推荐方式
- 也可以直接下载GitHub Workflow[自动利用Nuitka构建的版本（点击跳转）](https://github.com/wszqkzqk/pypvz/releases/tag/Latest)：
  - 使用MSVC编译
  - 每次提交均会更新，保证更新及时
  - 推荐方式
