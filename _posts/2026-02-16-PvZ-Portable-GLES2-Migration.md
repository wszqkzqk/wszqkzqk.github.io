...
catalog:      true
tags:         C++ OpenGL 开源软件 游戏移植 开源游戏 渲染
---
...
扩展声明虽然不影响运行时性能（它们只是函数指针定义），但 `gladLoadGLES2()` 初始化时会逐个查询每个扩展是否可用。去掉不需要的扩展可以略微加快初始化速度，更重要的是减少了编译时间——header-only 意味着每次 include 都要编译整个头文件。
...
这次迁移的核心思路可以概括为一句话：**用最小公共子集实现最大兼容性**。OpenGL ES 2.0 恰好是那个"最小公共子集"——它足够强大以满足 PvZ 的渲染需求，又足够基础以被所有目标平台支持。
