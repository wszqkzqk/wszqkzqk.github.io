...
其中R是气体常数，fc是渗透阈值。对于强熔体，如SiO<sub>2</<sub>sub>，上式中的渗透阈值是3D空间中的通用Scher–Zallen临界密度，即f<sub>c</sub> = 0.15，但对于脆性材料，渗透阈值是材料依赖的，且f<sub>c</sub> << 1。[^38] 键的H<sub>d</sub>和熵S<sub>d</sub>可以从可用的粘度实验数据中得到。
