...
在之前的[项目总览](https://wszqkzqk.github.io/2026/01/26/PvZ-Portable/)和 [GLES2 渲染后端迁移](https://wszqkzqk.github.io/2026/02/16/PvZ-Portable-GLES2-Migration/)中，笔者介绍了 PvZ-Portable 如何将《植物大战僵尸：年度版》的引擎带向各大平台。其中，GLES2 迁移尤其关键——将渲染后端统一到 OpenGL ES 2.0 这一最小公共子集，使得 Android 适配在图形层面成为可能。
...
要研究或使用此项目，你**必须**拥有正版 PC 版《植物大战僵尸：年度版》（GOTY Edition）的资源文件（如果没有，请在 [Steam](https://store.steampowered.com/app/3590/Plants_vs_Zombies_GOTY_Edition/) 或 [EA 官网](https://www.ea.com/games/plants-vs-zombies/plants-vs-zombies)上购买）。你需要从正版游戏中提取 `main.pak` 和 `properties/` 目录，通过应用内的导入功能导入到 PvZ-Portable 中。
...
|:----:|:----:|
| 在桌面长按图标可进入数据管理界面 | 数据管理界面可导入资源或管理存档 |
