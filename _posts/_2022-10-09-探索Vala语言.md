...
可见，Vala语言将这个在C99及以后的C语言标准中都语法正确的for循环转化成了一个较为复杂的while循环，并且是`while (TRUE)`和`if (!condition) {break;}`的结构。Vala编译器还将`i+=1`编译到了一个条件判断内，以便控制在首次循环时不执行`i+=1`。这些都使得Vala生成的C代码较为冗长，可读性较低。
