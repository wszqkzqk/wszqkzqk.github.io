...
if len(argv) == 1:
    ...
    if ((path[0] == "'") or (path[0] == '"')) and ((path[-1] == "'") or (path[-1] == '"')):
        path = path[1:-1]
    refpath = argv[2].replace('\\', '/')
    if ((refpath[0] == "'") or (path[0] == '"')) and ((path[-1] == "'") or (path[-1] == '"')):
        refpath = path[1:-1]
elif len(argv) == 4:
    inner = argv[1]
    path = argv[2].replace('\\', '/')
...
if len(argv) == 2:
    if (argv[1] == '--help') or (argv[1] == '-h'):
        ...
        print('      -- 将目标文件中所有以"#~"开头且以".png", ".jpg", ".jpeg", ".svg", ".gif"结尾的行表达为markdown图片格式（注意图片末不能有空格）')
        print('2. mdimginsert.py')
        print('      -- 输入目标文件路径，将其中以"#~"开头"#~"且以".png", ".jpg", ".jpeg", ".svg", ".gif"结尾的行表达为markdown图片格式（注意图片末不能有空格）')
...
for i in range(len(content)):
    ...
    if flag:
        ...
        if '://' in imgname:
            content[i] = '[![#~{0}]({1})]({1})'.format(os.path.basename(imgname), imgname) + '\n'
        else:
            content[i] = '[![#~{0}](/img/{0})](/img/{0})'.format(imgname) + '\n'
