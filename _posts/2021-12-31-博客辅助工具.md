...
1. mdimginsert.py [待处理的.md文件的路径]
      -- 将目标文件中所有以"#~"开头且以".png", ".jpg", ".jpeg", ".svg", ".gif"结尾的行表达为markdown图片格式（注意图片末不能有空格，仅支持站内图片）
2. mdimginsert.py
      -- 输入目标文件路径，将其中以"#~"开头"#~"且以".png", ".jpg", ".jpeg", ".svg", ".gif"结尾的行表达为markdown图片格式（注意图片末不能有空格，仅支持站内图片）
