...
首先，与太阳计算一样，我们需要计算从 J2000.0 历元（2000 年 1 月 1 日 12:00 UTC）起算的儒略世纪数 $T$。在 Vala 中，我们利用 GLib 提供的 `GLib.Date` 类来方便地处理日期，并将其转换为儒略日。此外 GLib 的 `Date.get_julian ()` 方法返回的儒略日是从公元1年1月1日开始计算的（注意不是常见的公元前4713年），我们需要将其转换为相对于 J2000.0 的天数。
...
这里不仅包含了摄动项，还直接体现了月球轨道的几何特征。第一项系数 $5.1282^\circ$ 主要对应月球轨道相对于黄道的平均**倾角 (Inclination)**（约为 $5.145^\circ$）。由于 $F$ 包含了**交点退行 (Regression of nodes)** 的影响（即升交点在黄道上向西移动，周期约 18.6 年），这一项描述了月球在黄道面上下摆动的基本运动。
...
本程序采用了 **[Saemundsson 公式](https://en.wikipedia.org/wiki/Atmospheric_refraction#Calculating_refraction)** 来估算标准大气条件下的折射角 $R$（单位：度）：
