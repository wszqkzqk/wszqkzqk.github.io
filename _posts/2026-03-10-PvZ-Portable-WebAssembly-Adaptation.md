...
这是 PvZ-Portable 目前为止**最复杂的一次平台适配**。Android 需要编写 Java Activity 和 JNI 桥接，iOS 需要处理签名和侧载，但它们本质上都是把同一个 SDL2 程序交叉编译到另一个操作系统。而 WebAssembly 面临的是一个完全不同的执行模型——**浏览器没有线程（主线程不能阻塞）、没有本地文件系统、没有 `nanosleep`、没有传统的主循环**。几乎每一个底层假设都需要重新审视。
