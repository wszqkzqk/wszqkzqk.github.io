...
这段代码的问题在于**边界检查的时序**。`memcpy` 发生在容量判定之前，这意味着如果某一次调用传入的顶点数量直接超过了数组剩余容量，数据会在边界检查触发 flush 之前就写到了数组末尾之外。这是明确的未定义行为，在不同运行环境下的表现差异很大——有时只是静默破坏相邻堆内存，表现为偶发的卡顿或完全无关模块的异常崩溃；有时则会直接触发段错误。随着 PvZ-Portable 在 AddressSanitizer、WebAssembly 和移动平台上的测试增多，这种不稳定性终于从偶发变成了可复现。
...
问题在于，当一个复杂的粒子特效（比如大量碎片或爆炸火花）触发裁剪时，单个输入三角形可能被切分成数十个子三角形。如果此时 `mTriangleCount` 已经接近 `MAX_TRIANGLES`，循环内没有任何容量检查，最终一定会写穿 `mVertArray` 的边界。

这个问题在移动端尤其致命，因为移动 GPU 的 driver 通常对越界访问更加敏感。修复非常直接：在每次增量前检查容量，若已满则先 `DrawGroup(g)` 提交当前积攒的三角形：
...
这次对渲染层缓冲区安全的集中修复，本质上是对底层代码中遗漏边界和生命周期管理的系统补齐。这些缺陷在过去可能因为触发概率低或特定平台的容错机制而没有立刻暴露，但它们始终是悬挂在稳定性之上的定时炸弹。

从 `gVertices` 的裸数组到 `std::vector` 的迁移，不只是语法层面的现代化，更重要的是把边界保护和生命周期管理从程序员的记忆里转移到语言机制中。`EffectSystem` 的 `MAX_TRIANGLES` 检查和 `VertexList` 的 `delete[]` 修复，则是对历史代码中遗漏边界的逐一补齐。这些改动看起来细碎，但共同构成了引擎在复杂场景下稳定运行的基础。
