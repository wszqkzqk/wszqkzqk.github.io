...
`generate_sun_angles` 函数是应用计算的核心。笔者在此实现了 Meeus 算法的等价变体，通过精确的天体力学模型计算太阳位置。算法的主要参考来源是 [Paul Schlyter 和 J. Giesen 总结的高精度算法页面](http://www.jgiesen.de/elevaz/basics/meeus.htm)。这个算法较简单，但具有很高的精度，适合我们这个应用的需求。
