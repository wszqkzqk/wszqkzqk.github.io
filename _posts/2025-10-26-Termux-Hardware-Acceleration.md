...
利用MediaCodec和FFmpeg在Termux中进行全面硬件加速的视频编解码实测表现出色，实现了极高的视频处理的效率和能效。这种转码方法可谓是非常环保。笔者成功使用手机将若干保存的AVC编码的高清电影转码为HEVC编码，通过控制质量因子，**节省了大量存储空间**；同时，仅利用了手机闲置待机时间完成了转码任务，电量开销也不大，全硬件编解码场景中手机**完全没有可感知发热**，**质量**观感也比5800H的硬件编码**更优**。这可以作为高效利用闲置资源的一个范例。
