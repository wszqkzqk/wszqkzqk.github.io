...
在上述优化之后，虽然启动速度已经很快，但对于文件系统的系统调用（Syscall）仍然有优化的空间。笔者进一步引入了**目录级的负面缓存（Negative Directory Cache）**。
...
static bool CheckSinglePath(std::string_view thePath)
{
    std::string aDir = GetFileDir(thePath);
    
    // 0. 查缓存：如果你爹都不在，那你肯定也不在
    ...
    // 如果文件不存在，顺便检查一下目录是否存在
    if (!FileExists(aDir)) {
        gMissingDirCache.insert(aDir); // 记下来，下次别来了
    }
...
这个改动进一步减少了在探测不存在的资源变体（Variant）时产生的 `stat` 系统调用次数，特别是在资源路径层级较深的情况下效果显著。

## 最终成果

经过这一系列优化，PvZ-Portable 在 Arch Linux (Ryzen 5800H) 上的平均启动时间最终稳定在 **1.5s - 1.7s**。

第二阶段的排查和优化甚至将龙芯 3C5000L 上的启动时间在第一阶段的基础上减半，从 **4.8s** 降到了 **2.4s**。更令人欣慰的是，`-O2` 和 `-O3` 构建之间的性能差距被缩小到了 **50ms** 以内。这表明我们成功消除了大部分低效的冗余计算代码，性能不再呈现出明显的 CPU 瓶颈和编译器优化依赖。

经过第三阶段的目录级负面缓存优化后，启动时间基本被压缩到了极致，在搭载 Ryzen 5800H 的 Arch Linux 上启动时间稳定在 **1.5 - 1.7s**，在龙芯 3C5000L 上则下降到了 **2.2s - 2.3s**。这一阶段对 Linux 平台的提升有限，因为之前已经对 Linux 下的情况进行了较多优化。但对于 Windows 和 Wine，这一阶段效果十分明显，同样是在 Ryzen 5800H 上，Wine 下的启动时间从 **4.5s** 降到了 **3.1s - 3.2s**。
...
3.  **Stage 3**: 通过目录级负面缓存，避免了重复的无效系统调用。
