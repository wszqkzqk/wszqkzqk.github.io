...
随着笔者的更多博客逐渐被搜索引擎所收录，笔者自建的RAG系统的意义正在逐渐减小。

笔者实测发现，使用[Grok网页版](https://grok.com/)并选择Grok 4 Fast模型，将[本博客网站链接](https://wszqkzqk.github.io/)加到提示词中，模型即会自动检索并引用笔者博客中的内容来回答问题。此外，该模型在网页版中自带的网页搜索功能也比笔者实现的要强大得多，它使用Agent处理搜索，一定程度上克服了传统RAG的局限性。这种方法更为强大灵活但Token开销更大，不过由于服务由模型提供方承担，他们事实上并不在乎这一点。
