...
catalog:    true
tags:       AI LLM 开源软件 archlinux LoongArchLinux OpenWebUI RAG
---
...
随着笔者的更多博客逐渐被搜索引擎所收录，笔者自建的RAG系统的意义正在逐渐减小。传统的RAG具有一定的局限性，比如单一检索机制，无法处理复杂查询，缺乏智能协调与额外行动，对大规模或多源数据的适应性差等。使用更为复杂的Agent机制可以在一定程度上克服这些局限性。

笔者实测发现，使用[Grok网页版](https://grok.com/)并选择Grok 4 Fast模型，将[本博客网站链接](https://wszqkzqk.github.io/)加到提示词中，模型即会利用Agent自动检索并引用笔者博客中的内容来回答问题。此外，该模型在网页版中自带的网页搜索功能也比笔者实现的要强大得多，它使用Agent处理搜索，一定程度上克服了传统RAG的局限性。这种方法更为强大灵活但Token开销更大，不过由于服务由模型提供方承担，他们事实上并不在乎这一点。

Grok默认即打开了强大的网页搜索功能，它不仅会自动生成搜索关键词查询，还会自动判断内容是否相关、资料是否充足且恰当。因此，除了笔者编写的Arch Linux for Loong64文档外，它还能很好地利用上更多的网络资源来回答问题，往往能带来更好的效果。

由于目前Grok网页版中的Grok 4 Fast已经完全开放给免费用户不限次数使用，笔者也十分推荐大家直接使用Grok网页版并附上[本博客的链接](https://wszqkzqk.github.io/)以及[补丁集仓库的链接](https://github.com/lcpu-club/loongarch-packages)来查询Arch Linux for Loong64相关的问题。尤其是对于不能访问北大校园网的用户来说，使用Grok网页版辅助维护是一个非常好的选择。

另外，对于补丁集的已有内容，笔者也利用DeepWiki提供了一份自动生成的[结构式文档与问答系统](https://deepwiki.com/lcpu-club/loongarch-packages)，无论是否在北大校园网内都可以访问，用户可以通过该系统快速获取已有的修复参考和维护惯例。
