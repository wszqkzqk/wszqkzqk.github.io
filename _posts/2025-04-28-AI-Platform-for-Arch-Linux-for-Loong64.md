...
随着笔者的这些博客逐渐被搜索引擎所收录，笔者自建的RAG系统的意义正在逐渐减小。

笔者实测发现，使用Grok网页版并选择Grok 4 Fast模型，将本博客网站链接加到提示词中，模型即会自动检索并引用笔者博客中的内容来回答问题。此外，该模型在网页版中自带的网页搜索功能也比笔者实现的要强大得多。Grok默认即打开了强大的网页搜索功能，它不仅会自动生成搜索关键词查询，还会自动判断内容是否相关、资料是否充足且恰当。因此，除了笔者编写的Arch Linux for Loong64文档外，它还能很好地利用上更多的网络资源来回答问题，这往往能带来更好的效果。

由于目前Grok网页版中的Grok 4 Fast已经完全开放给免费用户不限次数使用，笔者也十分推荐大家直接使用Grok网页版并附上本博客的链接以及补丁集仓库的链接来查询Arch Linux for Loong64相关的问题。尤其是对于不能访问北大校园网的用户来说，使用Grok网页版辅助维护是一个非常好的选择。
