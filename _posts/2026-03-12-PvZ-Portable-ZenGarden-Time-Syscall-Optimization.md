---
layout:       post
title:        PvZ-Portable：禅境花园性能优化——实现应用层系统调用缓存并修复 Wine 上游回归
...
起初，**笔者并没有立刻去 bisect Wine 的代码来盲查回归的 commit**，而是首先通过对程序自身的性能分析与优化，直接揭示了 Wine 的系统调用层发生了性能回归。在应用层面通过逐帧缓存完美绕过该问题后（这实际上已经完全解除了 PvZ-Portable 自身的性能危机），笔者并没有止步于此，而是决定“多做一步”：顺藤摸瓜构建了最小复现程序，最终定位到了 Wine 底层源码中的逻辑缺陷，并成功向 Wine 上游提交了修复补丁（已合并）。

本文将完整记录这次从发现卡顿到彻底解决的全过程。需要强调的是，这里其实包含了**两个维度的修复**：
1. **应用层的修复**：优化 PvZ-Portable 中对 `time` 等系统调用的需求，将数百次零碎调用合并为逐帧缓存。这已经足以彻底解决游戏本身的卡顿。
2. **上游生态的修复**：定位并修复了 Wine 11.4 引入的底层 `time` 系统调用性能衰退。这虽然对已经优化好的 PvZ-Portable 不再是刚需，但能够惠及广泛依赖时间函数的其他所有 Windows 程序，让它们在 Wine 下重获流畅。

这两步不仅展示了性能分析方法论与优化实现细节，还记录了如何排查 Wine 的底层源码，并将这一严重的性能回归彻彻底底修补在源头。
...
- **即时可行**——不需要一开始就编译数十个 Wine 版本进行盲目的二分测试
- **附带收益**——排查过程的副产品就是一个对所有平台都有效的优化补丁，无论 Wine 何时修复这个回归，游戏自身都已受益
...
单纯从解决 PvZ-Portable 禅境花园卡顿的问题来说，在完成了整个应用层的优化和代码审计后，目标其实已经完全达成了。通过逐帧缓存，游戏帧率目前已经毫无瑕疵地恢复。

但到此停止并不符合开源社区的极客精神：只要在生态中发现了明显的退化，**自己不卡了就行了显然不够**。在通过应用级修复精确定位到回归原点是时间相关的系统调用后，接下来就到了更进一步惠及他人的阶段：去写最小复现程序，去深入 Wine 源码找原因，并直接向 Wine 上游提交修复补丁。

## 后续：深入上游寻找根因与 Wine Bug 提交
...
提交报告后，笔者并不满足于只在自己的程序中绕过这个性能坑，决定深入 Wine 源码，亲自找出导致这次回归的本质原因。

通过对 Wine 源码的追踪，笔者发现问题出在 `dlls/ntdll/unix/system.c` 中的 `get_timezone_info()` 函数。在 Wine 11.4 周期的一个补丁中（commit `bfae6eb8b4f`），为了修复某些时区下逆向夏令时（inverse DST）的错误，上游在 `get_timezone_info()` 的热路径（hot path）上加入了无条件的 `mktime()` 检测：
...
因为 `time()`/`localtime()` 等很多底层逻辑都依赖 `get_timezone_info()`，导致在原本应该快速命中的缓存路径上，每调用一次时间函数都要强行执行两次非常昂贵的 `mktime()`，最终酿成了 25 倍的性能灾难。

明确原因后，笔者立刻编写了修复补丁。核心思想非常简单但关键：**遵循性能优化的基本法则，将昂贵的 libc/syscall 成本移出 cache hit 的快速路径**。

对于缓存键，笔者只依赖原本低开销的静态 `bias` 以及当前年份进行判断；只有发生缓存未命中进入慢路径初始化时，才重新计算 `inverted_dst`，从而在保留逆向夏令时修复效果的前提下，完全消除了热路径的性能惩罚。最后，笔者将代码改进至最简可读的形式，直接向上游提交了这枚补丁（可参见 [Wine Merge Request #10348](https://gitlab.winehq.org/wine/wine/-/merge_requests/10348)）。由于改动精确命中了痛点且重构清晰，该 MR 目前**已被成功并入**。

从下个 Wine 版本（11.5）开始，所有依赖时间函数的 Windows 应用都将受益于这个修复。换句话说，虽然我们最初的目的是解决 PvZ-Portable 的卡顿（并且已经在应用层完美解决了），但我们对这一问题的一查到底，最终让成千上万的其他老程序也幸免于难！

## 结语

这次优化的完整链条，展示了从自扫门前雪到惠及生态的全过程：

1. **发现**：Wine 11.4 下禅境花园主画面帧率骤降至 ~5 FPS
2. **分析**：通过分层计时剖析，逐步将瓶颈锁定为 `time(0)` / `localtime()` 在逐植物逐帧循环中的重复调用
3. **应用层修复**：在 `ZenGarden` 类中增加逐帧时间缓存 `mNowTime` / `mNowTM`，将每帧约 300 次系统调用降至 2 次
4. **验证**：帧率从 ~5 FPS 恢复到满帧，性能提升约 90 倍。至此，游戏本身的危机已经完全解除
5. **发现回归**：优化效果反向印证了 Wine 11.4 在时间系统调用层存在性能回归
6. **量化回归**：编写最小复现程序，定量测量出 Wine 11.4 中 `time()` / `localtime()` 的单次调用开销较 11.3 暴增约 25 倍
7. **提交报告**：向 Wine Bugzilla 提交了 Bug 59514
8. **生态层修复**：不满足于游戏不卡就罢休，进一步从 Wine 源码中揪出了 `get_timezone_info()` 缓存热路径中多余的 `mktime()` 调用，直接提交修复补丁并被成功合并，挽救了整个 Wine 生态中依赖时间函数的应用性能

这个案例证明了**有时候，深度剖析并修复你自己的应用问题，就是为上游生态作出贡献的最好契机**。当你不仅解决了自身的问题，还能再多走一步、追根溯源，就能让更多的人受益。
