...
**笔者没有通过 bisect Wine 的代码来找到回归的 commit**，而是通过对程序自身的性能分析与优化，直接揭示了 Wine 的性能回归所在：当你从程序层面就能消除整类系统调用的热路径开销时，回归的根因自然就呼之欲出了。
...
从下个 Wine 版本（11.5）开始，所有依赖时间函数的应用都将受益于这个修复，无论是 PvZ-Portable （虽然已经优化了）还是其他任何使用 `time()` / `localtime()` 的程序，都将避免这次回归带来的性能问题！
...
8. **上游修复**：定位到底层 `get_timezone_info()` 缓存热路径中的两次多余 `mktime()` 调用，提交修复补丁并被成功合并，挽救了所有依赖该时间函数的 Wine 应用性能

这个案例证明了**有时候，修复你自己的程序就是定位第三方回归最高效的方式**。当你对热路径有足够的理解时，性能剖析数据本身就是最好的证据。
