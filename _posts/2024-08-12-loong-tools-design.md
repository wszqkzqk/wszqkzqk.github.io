...
目前，Vala上游已经修复了（其实是Workaround）这个问题，在`0.56.15`版本及以后的Vala在编译代码时并不会因这一问题而失败，我们并**不需要维护**这个patch。然而，现存于仓库的`vala`软件包是`0.56.14`版本，在`gcc`升级到`14`之前完全可以正常使用，但是现在升级到`gcc 14`后`vala 0.56.14`产生的C代码则无法通过编译，也就**无法完成自举**，即使上游已经修复，我们也仍然需要处理这个问题。
