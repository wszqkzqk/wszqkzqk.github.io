...
| Hash集合 | GLib.GenericSet | Gee.HashSet | hash_set | GLib实现必须自己指定hash函数和相等判断函数；Gee实现中默认`string`类型是比较内容，其余类型均是比较内存地址，比较内容需要手动指定函数 |
| 平衡二叉树集合 |  | Gee.TreeSet | set | GLib实现必须自己指定比较函数；Gee实现中默认`string`类型是比较内容，其余类型均是比较内存地址，比较内容需要手动指定函数 |
| Hash字典 | GLib.HashTable | Gee.HashMap | hash_map | GLib实现必须自己指定hash函数和相等判断函数；Gee实现中默认`string`类型是比较内容，其余类型均是比较内存地址，比较内容需要手动指定函数 |
