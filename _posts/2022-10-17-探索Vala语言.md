...
Vala的设计即为现代化的高级语言，拥有大量现代语言的功能。此外，Linux平台下使用最广泛、功能集成最丰富的C语言库——GLib可以认为是Vala语言的标准库。GLib内置了极其丰富的内置函数、GObject对象、IO通道、数据结构，因此，Vala语言内置的标准库也十分强大。下面笔者将对一些自己喜欢的Vala内置的强大功能进行简要介绍。
