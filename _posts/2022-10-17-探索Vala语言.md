...
Vala对当前是否处于交互文本界面的检测目前有一些平台依赖的Bug。Windows平台下，Vala的报错提示在默认情况下通常不是彩色的，需要在编译时加入`--color=always`参数才能开启彩色报错。

笔者已经提交[相关补丁](https://gitlab.gnome.org/GNOME/vala/-/merge_requests/292)修复此问题，使得Windows下也能默认彩色报错，不再需要自己加`--color=always`参数，但该补丁尚未合并。
