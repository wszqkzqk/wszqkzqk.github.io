...
虽然Vala语言有专门为Vala设计的容器库Gee，但是其实在GLib中就已经集成了很多的容器类型，一般情况下已经可以满足使用。

笔者个人认为，由于GLib的安装与应用远远比Gee广泛，得到的测试也更多，而且在Vala中应用时不用引入额外的依赖，虽然有部分与Vala耦合不好的地方，但笔者仍然推荐优先在Vala中使用GLib库中的容器。

这里列出**部分**GLib中的容器与Gee中的容器的对应关系（空缺不一定是没有，也可能是待补充），还列出了C++ STL中的对应类型作为对比与参考：
