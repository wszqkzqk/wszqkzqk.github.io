...
Vala中一般可以选择使用两个容器库，一个是Vala程序本身即依赖的GLib，一个是专门为Vala设计的容器库Gee。两者各有一些优缺点：GLib库应用范围比Vala大，库内容本身得到的测试更加充分，更加可靠，但与Vala的集成相对不好，在集成与调用上容易出现问题；而Gee库本身即是由Vala所写，与Vala的集成更好，但是本身得到的验证与测试相对更少，可靠性相对更低。
