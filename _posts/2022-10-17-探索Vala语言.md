...
void main () {
    ...
    if (re.match (cc_err, 0, out match_info)) {
        var info = match_info.fetch (0);
        var info_array = info.split (" ");
        print (@"You are using $(info_array[0])!\n");
        print (@"The version of your $(info_array[0]) is: $(info_array[2])\n");
        ...
        var ver = info_array[2].split (".");
        var main_ver = uint.parse (ver[0]);
        var sub_ver = uint.parse (ver[1]);
...
然而，笔者个人认为，由于GLib的安装与应用远远比Gee广泛，得到的测试也更多，而且在Vala中应用时不用引入额外的依赖，虽然有部分与Vala耦合不好的地方，但笔者仍然推荐优先在Vala中使用GLib库中的容器。

这里列出**部分**GLib中的容器与Gee中的容器的对应关系（空缺不一定是没有，也可能是待补充）：

| 容器类型 | GLib实现 | Gee实现 | C++ STL实现（对比） | 备注 |
| :---: | :---: | :---: | :---: | :---: |
| 列表 | GLib.Array | Gee.ArrayList | vector |  |
| 双端链表 | GLib.List |  | list |  |
| 单向链表 | GLib.SList |  | slist |  |
| Hash集合 | GLib.GenericSet | Gee.HashSet | hash_set | GLib实现必须自己指定hash函数和相等判断函数；Gee实现中默认除了`string`类型是比较内容，其余类型均是比较内存地址，否则需要手动指定 |
| 平衡二叉树集合 |  | Gee.TreeSet | set | GLib实现必须自己指定比较函数；Gee实现中默认除了`string`类型是比较内容，其余类型均是比较内存地址，否则需要手动指定 |
| Hash字典 | GLib.HashTable | Gee.HashMap | hash_map | GLib实现必须自己指定hash函数和相等判断函数；Gee实现中默认除了`string`类型是比较内容，其余类型均是比较内存地址，否则需要手动指定 |
| 平衡二叉树字典 | GLib.Tree | Gee.TreeMap | map | GLib实现必须自己指定比较函数；Gee实现中默认除了`string`类型是比较内容，其余类型均是比较内存地址，否则需要手动指定 |
| 双端队列 | GLib.Queue | Gee.Deque | deque |  |
...
#### 二进制`0b`与八进制`0o`整数表达式（待合并）

笔者为Vala提供了对以`0b`开头的二进制整数表达式，以及以`0o`开头的八进制整数表达式的支持。该功能已经提交了[PR](https://gitlab.gnome.org/GNOME/vala/-/merge_requests/273)，但目前尚未合并。这支持了以下写法：
...
其中，二进制整数表达式为新增写法，而以`0o`开头的八进制整数表达式的可读性较原来以单个`0`开头的八进制表达式可读性更好。
