...
void main() {
    ...
    if (msys2_dep_regex.match(path, 0, out match_info)) {
        ...
        print("This file doesn't in msys2.\n");
    }
    // Output:
...
笔者为Vala下一个版本提供的[逐字字符串模板功能](https://gitlab.gnome.org/GNOME/vala/-/merge_requests/266)则已经得到合并，该功能支持将Vala的逐字字符串与模板字符串结合使用，其关系类似于Python的`rf""`之于`r""`和`f""`，进一步增强了Vala的字符串处理功能，该功能支持这样的表达：
```vala
void main () {
    double pi = 3.141592653589;
    var s = @"""\pi/ \wow/ \$pi/""";
