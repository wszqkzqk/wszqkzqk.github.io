...
Python程序员需要注意，Vala中的`split`与Python有些区别：对于多个相连的分隔符的`split`，在分隔的结果中，Python不会用空字符串表示两个分隔符直接的内容，而Vala会保留所得的空字符串。例如对于Python：（以下示例字符串中均有4个空格，博客构建器存在bug，无法正常显示）
```python
tab = "1    2".split()
```
