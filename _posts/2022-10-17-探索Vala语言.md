...
另外需要注意的是，字符串切片可以使用负数索引，`-n`的意义为倒数第`-n+1`个字符的位置；但从字符串中提取字节时负数索引的含义为内存地址相对与字符串第一个字节的关系，直接用形如`s[-n]`获取字节将会越界，产生未定义行为，故一般情况下从字符串中提取字节时不应使用负数索引。
```vala
void main () {
    var s1 = "abcdefg";
    var s2 = s1[1:-1]; // "bcdef"
...
- [在class或者struct中不能在同一行定义多个变量（未合并）](https://gitlab.gnome.org/GNOME/vala/-/merge_requests/291)
