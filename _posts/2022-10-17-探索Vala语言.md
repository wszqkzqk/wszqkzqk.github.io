...
```vala
void main() {
    var reg = new Regex(""".*(/|\\)(usr|ucrt64|mingw64|clang64|mingw32|clang32|clangarm64)(/|\\)""");
...
Vala虽然支持负数索引，但很多时候支持并不好。例如，在`GLib.Array`中，数组的索引并不支持负号。而且一般情况下，`a[-n]`没有按照惯例表示倒数第n个元素，而是表示倒数第n-1个元素，这使得最后一个元素不能通过`a[-1]`来获取。所以在使用Vala时，最好还是用`a[a.length-m]`来起到表示负数索引的效果。
