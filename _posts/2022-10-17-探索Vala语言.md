...
Python程序员需要注意，Vala中的`split`与Python有些区别：对于多个相连的分隔符的`split`，在分隔的结果中，Python不会用空字符串表示两个分隔符直接的内容，而Vala会保留所得的空字符串。例如对于Python：
```python
tab = "1    2".split()
```

以上得到的`tab`内容为`["1", "2"]`。而对于Vala：
```vala
var tab = "1    2".split(" ");
```

以上得到的`tab`内容为`{"1", "", "", "", "2"}`。有时忘记处理空字符串会引发某些bug。
