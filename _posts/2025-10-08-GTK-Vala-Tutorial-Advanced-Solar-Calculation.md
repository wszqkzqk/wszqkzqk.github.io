...
这是笔者早期使用的 Vala 函数，它基于 Spencer 的傅里叶级数来近似太阳赤纬和方程时。这种方法实现相对简单，但精度有限，尤其是在长期时间跨度上。
...
维基百科上介绍的公式是根据实际轨道参数对理想化的圆形轨道进行修正，来逼近真实的太阳位置。

* 从一个理想化的“平均太阳”出发，这个“平均太阳”在完美的圆形轨道上以恒定速度运动。它的位置可以用一个简单的余弦函数来描述：`cos((360°/365.24) * (N + 10))`，其中 `N` 是年积日，`10` 是从冬至到1月1日的大致天数。
* 对上述理想模型加入最重要的修正项——由地球轨道椭圆形（偏心率为 `e ≈ 0.0167`）引起的速度变化。这个修正被称为**中心差 (Equation of Center)**，其一阶近似为 `(360°/π) * e * sin(M)`，其中 `M` 是平近点角。维基百科的公式中 `1.914° * sin(0.98565° * (N - 2))` 正是这个修正项的体现。
* 将修正项代入，即可得到一个更精确的太阳黄经表达式，再通过坐标变换得到赤纬。

因此，这个公式是在一个简化的物理模型基础上，加入了关键的一阶修正，从而在不进行复杂计算的情况下，获得一个尚可的精度。

**赤纬 (δ):**
这个公式实际上是先计算了近似的太阳黄经 `λ`，然后通过 `δ = arcsin(sin(ε) * sin(λ))` 得到。维基百科给出的合并形式如下：
...
**方程时 (EoT):**
同样，方程时也可以通过一个包含两个主要周期项（分别对应轨道偏心率和黄赤交角影响）的简化公式来近似：
