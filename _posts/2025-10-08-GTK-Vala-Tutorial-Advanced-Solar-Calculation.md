...
维基百科上介绍的公式并非简单的数值拟合，而是从天体力学模型推导出的简化解析解。它根据实际轨道物理参数，对理想化的圆形轨道进行修正，来逼近真实的太阳位置。其核心思想是：

1.  从一个理想化的“平均太阳”出发，这个“平均太阳”在完美的圆形轨道上以恒定速度运动。
2.  对上述理想模型加入最重要的修正项——由地球轨道椭圆形（偏心率）引起的速度变化。这个修正被称为**中心差 (Equation of Center)**。
3.  将修正项代入，即可得到一个更精确的太阳黄经表达式，再通过坐标变换得到赤纬。
...
其中各参数的物理意义如下：
*   `-23.44°`：地球的**黄赤交角 `ε`**，决定了太阳赤纬的最大变化范围。
*   `N`：从当年1月1日午夜UT起算的**年积日**（例如，1月1日 N=0，1月2日 N=1）。
*   `360°/365.24`：地球公转的**平均角速度**（度/天）。
...
*   ` (360°/π) * 0.0167 * sin(...) `：此项是**中心差**的一阶近似，用于修正因轨道偏心率导致太阳视运动速度不均匀的问题。
...
*   `9.863*sin(2D + ...)`：此项是主要由**黄赤交角**引起的周期性变化，周期为半年。
