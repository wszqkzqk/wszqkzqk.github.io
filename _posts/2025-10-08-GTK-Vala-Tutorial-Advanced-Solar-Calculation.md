...
*  **求解日出/日落时角 (ω₀)**：将太阳高度角公式反解，求解时角 `HA`。设 `α` 为地平线修正角（一般取-0.83°，也可以按特殊需要调整），`φ` 为本地纬度，`δ` 为太阳赤纬，则日落时的时角 `ω₀` 满足：
...
| 性能指标 (单位: 度)    | **Meeus 算法 (当前实现)** | **傅里叶级数算法 (旧实现)** | **维基百科算法** |
| :--------------------- | :-------------------------: | :-----------------------------: | :----------------: |
| **最佳表现 (最小RMSD)** |         **`0.0001`**        |             `0.0010`            |      `0.0001`      |
| **最差表现 (最大RMSD)** |         **`0.0083`**        |             `0.3326`            |      `0.4171`      |
| **全场景平均RMSD (约)** |         **`~0.0034`**       |             `~0.083`            |      `~0.145`      |
...
| 地点       | Meeus平均 | Fourier平均 | Wikipedia平均 |
|------------|-----------|-------------|---------------|
...
* Meeus: 月度RMSD波动非常小，通常在0.00x的范围内。
