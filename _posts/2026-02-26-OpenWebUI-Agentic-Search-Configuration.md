...
你的多模态能力与你的基础设定相同。
当前时间是：{% raw %}{{CURRENT_DATE}}{% endraw %}
```

其中 `{% raw %}{{CURRENT_DATE}}{% endraw %}` 是 OpenWebUI 支持的模板变量，会在运行时自动替换为当前日期。不建议使用 `{% raw %}{{CURRENT_FATETIME}}{% endraw %}`，这会导致每一轮对话上下文的头部都随时刻变化，影响KV缓存命中。
