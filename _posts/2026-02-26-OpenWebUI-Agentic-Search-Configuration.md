...
subtitle:     基于 GUILess Bing Search 与 Qt Web Extractor 实现外部搜索引擎与网页加载集成
date:         2026-02-26
author:       wszqkzqk
...
随着大语言模型能力的不断提升，如何让模型有效获取并利用实时网络信息成为了各大 LLM 平台的核心关注点。传统的检索增强生成（RAG）方案虽然在一定程度上解决了模型知识时效性的问题，但其固有的**信息损失**依然显著：搜索结果经过分块、嵌入、向量检索等流水线处理后，模型最终只能看到**零散的片段**，而非页面的完整上下文，模型也无法在**信息不足时主动发起更多搜索**。
...
- **搜索决策**：OpenWebUI 根据用户提示词分析是否需要搜索
...
| 工具 | 功能 | 用途 |
| :--- | :--- | :--- |
| `fetch_page` | 加载并渲染网页，返回纯文本 | 通用网页内容提取，自动检测 PDF |
...
其中 `{{CURRENT_DATETIME}}` 是 OpenWebUI 支持的模板变量，会在运行时自动替换为当前时间。
