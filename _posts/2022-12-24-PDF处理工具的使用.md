...
date:       2022-12-24
author:     wszqkzqk
header-img: img/pdf-tools/PDF_file_icon.webp
catalog:    true
...
有时候我们还需要将PDF多页合并作一页，常见于打印页数很多的PPT的场景。这时候可以用`pdfjam`工具。`pdfjam`是基于LaTeX的`pdfpages`宏包的一个Shell脚本前端，通常包含在TeX Live发行版中。

在Arch Linux下，可以通过安装`texlive-binextra`包来获取（注意：不同时期包名可能有所不同，请查询确认）：
...
```bash
pdfjam --nup [列数]x[行数] [源文件].pdf --outfile [输出文件].pdf
```

例如，将`input.pdf`每2x2页合并为一张，输出到`output.pdf`：

```bash
pdfjam --nup 2x2 input.pdf --outfile output.pdf
```

如果需要横向排版，可以加上`--landscape`参数：

```bash
pdfjam --nup 2x2 --landscape input.pdf --outfile output.pdf
```

`pdfjam`功能非常强大，支持各种复杂的页面布局调整，更多用法可以通过`pdfjam --help`查看。
