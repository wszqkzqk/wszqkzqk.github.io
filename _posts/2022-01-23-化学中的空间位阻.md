...
此外，在环状分子中，常常还会有皮策应变。H.C. Brown曾将这些张力和跨环张力总结为内部张力。

脂环族化合物的许多反应，包括平衡、氧化还原和溶解反应，都是以反应中心的sp2和sp3状态之间的转变为特征的，与相应的应变能差SI（sp2-sp3）相关。[10] 这些数据主要反映了中环中不利的邻接角，如随着SI的增加，酮的还原率严重增加（图1）。另一个例子是桥头甲苯的溶解，相应的溴化物衍生物（sp3）和作为过渡态sp2-模型的羧基离子之间的立体能量差[11] （图2）。
