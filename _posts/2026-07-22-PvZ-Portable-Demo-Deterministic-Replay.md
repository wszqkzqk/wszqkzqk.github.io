...
现在，这套系统的能力其实已经超出了调试工具的定位：它完全可以当作**游戏录像**来用。和视频录屏相比，`.dmo` 里只有输入事件、随机种子和时间信息，不含任何画面数据，体积极小。一段几分钟的游戏，录屏软件产出的是动辄几百 MB 量级的视频，而 `.dmo` 通常只有 KB 级，小了好几个数量级。更重要的是，游戏录制没有任何压缩，在播放的时候，引擎会把同一局游戏当场重新跑一遍：原生分辨率渲染，没有任何转码压缩的画质损失，换平台、换时区放出来都分毫不差。录一局精彩的无尽发给朋友，对方 `-play` 一下就能看到原汁原味的完整对局。
