...
Qt WebEngine 本质上是 Chromium 的封装，前端渲染和 JS 执行能力与现代浏览器完全一致。它的核心优势在于本身就是一个极其成熟及通用的跨平台基础构件。在 Windows 和 macOS 等平台上，只需引入对应的 Python 绑定包 PySide6，即可直接作为轻量的提取引擎工作，免去了繁琐的额外配置。

而在 Linux 平台下，它额外带来了得天独厚的优势：作为非常通用的基础 GUI 依赖，从 x86 到 LoongArch、RISC-V 等各种指令集的分支，几乎所有的主流发行版都已经把它在软件仓库中打包集成好了。如果在 Linux 下使用，它可以直接复用系统的动态库，无需再去第三方服务器下载动辄好几百兆的 Chromium 内核。只要用常规的包管理器备齐环境，部署本项目几乎不会带来任何额外的存储占用开销。
...
* 兼容 Open WebUI：支持直接作为 Open WebUI 的外部网页加载器使用，源码目录中也包含了作为对话自定义工具引用的脚本。服务端常驻后台后，能为各类 AI 平台提供统一的页面获取服务。
...
* 部署极简：不需要独立下载浏览器，提供 systemd 服务文件和基于 [AUR](https://aur.archlinux.org/packages/qt-web-extractor) 的部署方式，安装和后台启停都非常方便。
...
作为业务后端服务是工具最常见的使用方式。项目内置了 HTTP 服务器：

```bash
qt-web-extractor serve --host 127.0.0.1 --port 8766 --api-key mysecretkey
```

启动以后就可以在 Open WebUI 的管理后台中，将 Web Loader Engine 设置为 external，填写这个地址和配置好的 API Key。此后 Open WebUI 内的所有网页阅读和对话中的链接请求都会交由它进行渲染分析，因为能执行 JS 的缘故，也可以顺利避开部分只拦截纯 HTTP 请求的反爬机制获取到文本。
